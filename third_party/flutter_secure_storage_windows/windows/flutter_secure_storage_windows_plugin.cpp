...
namespace
{
  ...
  std::string WideToUtf8(const std::wstring& input) {
    ...
    WideCharToMultiByte(
        CP_UTF8, 0, input.c_str(), -1, output.data(), size_needed, nullptr, nullptr);
    output.resize(size_needed - 1);
    return output;
  }
...
#pragma warning(pop)
  ...
  flutter::EncodableMap FlutterSecureStorageWindowsPlugin::ReadAll()
  {
      ...
      do {
          ...
          const std::string key_utf8 = WideToUtf8(fileName);
          std::optional<std::string> val = this->Read(key_utf8);
          auto key = this->RemoveKeyPrefix(key_utf8);
          if (val.has_value()) {
              creds[key] = val.value();
              continue;
          }
